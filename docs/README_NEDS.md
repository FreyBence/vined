> **Archived / deprecated documentation.** This is the NEDS README as it existed
> in this checkout before the ViNED documentation rewrite, including local Bash
> command edits. It preserves the project origin and historical instructions.
> The schematic link was adjusted for this location. For the current project,
> see the [ViNED README](../README.md).
...
```bash
bash prepare_data.sh 1 EID    # Download data for a session using the provided EID

bash prepare_data.sh 84       # Download 84 sessions from the IBL repeated-site dataset
```

To accelerate data loading during training, we pre-save the partitioned train/val/test data and load it when needed:

```bash
bash create_dataset.sh 1 EID    # Save the train/val/test data for a session using the provided EID

source run_create_dataset.sh      # Save the train/val/test data for each of the 10 test sessions individually

bash create_dataset.sh 10 none  # Save the train/val/test data for pretraining on a set of 10 sessions
```
...
```bash
bash train.sh 1 EID train mm 0 0.1 False random        # Train multi-modal model

bash train.sh 1 EID train encoding 0 0.1 False random  # Train encoding model

bash train.sh 1 EID train decoding 0 0.1 False all     # Train decoding model
```

Use `Ray Tune` for hyperparameter search. Update `num_tune_sample` in the script to change the number of random models:

```bash
bash train.sh 1 EID train mm 0 0.1 True random
```
...
```bash
bash train_multi_gpu.sh 10 none mm 0 0.1 all   # Pre-training requires "all"
```

Fine-tune the pre-trained 10-session NEDS model on a single held-out test session:

```bash
bash train.sh 10 EID finetune mm 0 0.1 False random
```
...
```bash
bash eval.sh 1 EID train mm 0.1 random False     # Eval a model trained from scratch (no hyperparameter search)

bash eval.sh 10 EID finetune mm 0.1 random False # Eval a pre-trained model fine-tuned on a held-out test session (no hyperparameter search)

bash eval.sh 1 EID train mm 0.1 random True      # Eval a model trained from scratch (with hyperparameter search)
```
